<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="description" content="">
<meta name="author" content="">
<title>The Gong Lab -- Members</title>

<!-- Bootstrap Core CSS -->
<link href="static/css/bootstrap.min.css" rel="stylesheet">

<!-- Custom CSS -->
<link href="static/css/business-casual.css" rel="stylesheet">

<!-- Fonts -->
<link href="https://fonts.googleapis.com/css?family=Raleway:200,400" rel="stylesheet">
<link href="https://fonts.googleapis.com/css?family=Open+Sans:300italic,400italic,600italic,700italic,800italic,400,300,600,700,800" rel="stylesheet" type="text/css">

<!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
        <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
        <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->

</head>

<body>
<? include 'nav.inc' ?>
<div class="container">
  <div class="row" id="row1">
    <div class="box banner" id="members">
      <div class="col-lg-12 text-center">
        <div class="brand" id="custom-font">Members</div>
      </div>
    </div>
  </div>
  <div class="row">
    <div class="box">
      <div class="col-lg-12">
        <br>
      </div>
      <div class="col-lg-12 text-center"> <img src="static/img/portfolio/holder.jpg" class="img-responsive center-block" alt="">
        <p class="member"><span class="name">Xiaohua Gong</span><br />
          Professor<br />
          xgong@berkeley.edu</p>
      </div>
      <div class="col-lg-1 col-sm-1 col-xs-1 text-center"></div>
      <div class="col-lg-5 col-sm-5 col-xs-5 text-center"> <img src="static/img/portfolio/holder.jpg" class="img-responsive center-block" alt="">
        <p class="member"><span class="name">Chun-Hong Xia</span><br />
          Staff Scientist<br />
          chxia@berkeley.edu</p>
      </div>
      <div class="col-lg-5 col-sm-5 col-xs-5 text-center"> <img src="static/img/members/ML.jpg" class="img-responsive center-block" alt="">
        <p class="member"><span class="name">Mei Li</span><br />
          Staff Scientist<br />
          mei.li@berkeley.edu</p>
      </div>
      <div class="col-lg-1 col-sm-1 col-xs-1 text-center"></div>
      <div class="col-lg-12">
        <br>
      </div>
      <hr>
        <h2 class="intro-text text-center">Graduate Researchers<br />
        </h2>
      <hr>
      <div class="col-lg-12">
        <br>
      </div>
      <div class="col-lg-1 col-sm-1 col-xs-1 text-center"></div>
      <div class="col-lg-5 col-sm-5 col-xs-5 text-center"> <img src="static/img/members/JL.jpg" class="img-responsive center-block" alt="">
        <p class="member"><span class="name">Jimmy Liu</span><br />
          Postdoc<br />
          chunyenliu@berkeley.edu</p>
      </div>
      <div class="col-lg-5 col-sm-5 col-xs-5 text-center"> <img src="static/img/portfolio/holder.jpg" class="img-responsive center-block" alt="">
        <p class="member"><span class="name">Wiktor Stopka</span><br />
          Grad Student<br />
          wiktor.stopka@berkeley.edu</p>
      </div>
      <div class="col-lg-1 col-sm-1 col-xs-1 text-center"></div>
      <div class="col-lg-12">
        <br>
      </div>
      <hr>
        <h2 class="intro-text text-center">Undergraduate Researchers<br />
        </h2>
      <hr>
      <div class="col-lg-12">
        <br>
      </div>
    </div>
  </div>
  <div class="row">
    <div class="box">
      <div class="col-lg-12">
        <hr>
        <h2 class="intro-text text-center">Alumni </h2>
        <hr>
        <table class="table table-striped">
          <tr>
            <th>Name</th>
            <th>Position</th>
            <th>Current Position</th>
          </tr>
          <tr>
            <td>Eddie Wang</td>
            <td>Postdoc</td>
            <td>Emerald Cloud Lab</td>
          </tr>
          <tr>
            <td>Dong Wang</td>
            <td>Postdoc</td>
            <td>UCLA Postdoc</td>
          </tr>
          <tr>
            <td>Cathy Cheng</td>
            <td>Postdoc</td>
            <td>Scripps Research Institute</td>
          </tr>
          <tr>
            <td>Deepika Arora</td>
            <td>Postdoc</td>
            <td></td>
          </tr>
          <tr>
            <td>Haiquan Liu</td>
            <td>Postdoc</td>
            <td></td>
          </tr>
          <tr>
            <td>Kaijun Wang</td>
            <td>Postdoc</td>
            <td></td>
          </tr>
          <tr>
            <td>Meng Wang</td>
            <td>Postdoc</td>
            <td>Monsanto Company</td>
          </tr>
          <tr>
            <td>Ming-kai Lin</td>
            <td>Postdoc</td>
            <td></td>
          </tr>
          <tr>
            <td>Ying Chen</td>
            <td>Postdoc</td>
            <td></td>
          </tr>
          <tr>
            <td>Lin Li</td>
            <td>Grad Student</td>
            <td></td>
          </tr>
          <tr>
            <td>Alex Park</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Debra Cheung</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Hong Ma</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Jing Zeng</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Derrick Ko</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Elizabeth Sanders</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Joyce Hsieh</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Joycelyn Niimi</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Tarah Yee</td>
            <td>Research Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Andrew Geng</td>
            <td>Undergraduate Assistant</td>
            <td>Northwestern Med School</td>
          </tr>
          <tr>
            <td>Lucy Li</td>
            <td>Undergraduate Assistant</td>
            <td>Thomas Jefferson Med School</td>
          </tr>
          <tr>
            <td>Amy Chinn</td>
            <td>Undergraduate Assistant</td>
            <td>UC San Diego Biomedical Sciences Graduate Program</td>
          </tr>
          <tr>
            <td>Angela Shi</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Anna Zhou</td>
            <td>Undergraduate Assistant</td>
            <td>New York University School of Medicine</td>
          </tr>
          <tr>
            <td>Benjamin Chen</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Camille Ezran</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>David Zeltser</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Desiree Cadiz</td>
            <td>Undergraduate Assistant</td>
            <td>UC Berkeley School of Optometry</td>
          </tr>
          <tr>
            <td>Eliza Lee</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Eric Kuo</td>
            <td>Undergraduate Assistant</td>
            <td>Albert Einstein College of Medicine</td>
          </tr>
          <tr>
            <td>Eric Lin</td>
            <td>Undergraduate Assistant</td>
            <td>University of Pennsylvania School of Dental Medicine </td>
          </tr>
          <tr>
            <td>Eric Lu</td>
            <td>Undergraduate Assistant</td>
            <td>UC Davis School of Medicine</td>
          </tr>
          <tr>
            <td>Felicia Tang</td>
            <td>Undergraduate Assistant</td>
            <td>Case Western Reserve University School of Medicine</td>
          </tr>
          <tr>
            <td>Grace Kang</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>James Lee</td>
            <td>Undergraduate Assistant</td>
            <td>David Geffen School of Medicine at UCLA</td>
          </tr>
          <tr>
            <td>Jean Rao</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Jeffrey Tsao</td>
            <td>Undergraduate Assistant</td>
            <td>UC San Diego School of Medicine </td>
          </tr>
          <tr>
            <td>Jennifer Lee</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Lauri Shinn</td>
            <td>Undergraduate Assistant</td>
            <td>UC Los Angeles School of Dentistry</td>
          </tr>
          <tr>
            <td>Leslie Lan</td>
            <td>Undergraduate Assistant</td>
            <td>Johns Hopkins Carey School of Business</td>
          </tr>
          <tr>
            <td>Melissa Pun</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Nina Tsai</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Peggy Lee</td>
            <td>Undergraduate Assistant</td>
            <td>Johns Hopkins University School of Nursing</td>
          </tr>
          <tr>
            <td>Raymond Zhang</td>
            <td>Undergraduate Assistant</td>
            <td>Feinberg School of Medicine, Northwestern University</td>
          </tr>
          <tr>
            <td>Serena Tseng</td>
            <td>Undergraduate Assistant</td>
            <td>Research Associate at Stanford University</td>
          </tr>
          <tr>
            <td>Sheldon Leong</td>
            <td>Undergraduate Assistant</td>
            <td>UC Davis School of Medicine</td>
          </tr>
          <tr>
            <td>Stanley Ng</td>
            <td>Undergraduate Assistant</td>
            <td>NIH Neuroscience Researcher</td>
          </tr>
          <tr>
            <td>Stephanie Huang</td>
            <td>Undergraduate Assistant</td>
            <td>Roche Molecular Systems </td>
          </tr>
          <tr>
            <td>Vibha Mahendra</td>
            <td>Undergraduate Assistant</td>
            <td>Stanford University School of Medicine</td>
          </tr>
          <tr>
            <td>Vishaal Sridhar </td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>Wendi Gu</td>
            <td>Undergraduate Assistant</td>
            <td>UC San Francisco School of Medicine</td>
          </tr>
          <tr>
            <td>Yang Yang</td>
            <td>Undergraduate Assistant</td>
            <td></td>
          </tr>
          <tr>
            <td>David Lo</td>
            <td>Undergraduate Assistant</td>
            <td>New York University School of Medicine</td>
          </tr>
          <tr>
            <td>Peter Martindale</td>
            <td>Undergraduate Assistant</td>
            <td>UC Berkeley School of Optometry</td>
          </tr>
        </table>
      </div>
    </div>
  </div>
</div>
<!-- /.container -->

<footer>
  <div class="container">
    <div class="row">
      <div class="col-lg-12 text-center">
        <p><a href="https://www.ocf.berkeley.edu"> <img src="static/img/ocf.png" alt="Hosted by the OCF" style="border: 0;" /></p>
      </div>
    </div>
  </div>
</footer>

<!-- jQuery --> 
<script src="static/js/jquery.js"></script> 

<!-- Bootstrap Core JavaScript --> 
<script src="static/js/bootstrap.min.js"></script>
</body>
</html>
